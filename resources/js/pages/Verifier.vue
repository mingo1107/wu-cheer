    ...
    <div class="max-w-7xl mx-auto">
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl p-8 border border-white/20">
          ...
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
            <div>
              <h2 class="text-2xl font-bold text-gray-800 mb-2">核銷人員管理</h2>
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                  ...
                  <li>
                    <div class="flex items-center">
                      ...
                      <span class="text-sm font-medium text-gray-500">核銷人員管理</span>
                    </div>
                  </li>
                </ol>
              </nav>
          ...
          <div class="mb-6">
            <div class="flex flex-col sm:flex-row gap-4">
              ...
              <select
                ...
                @change="loadVerifiers"
                class="px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-all duration-200"
              >
          ...
          <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div v-else-if="dataList.length === 0" class="p-8 text-center">
              ...
              <p class="text-gray-600 text-lg">沒有找到核銷人員</p>
            </div>
            ...
            <div v-else class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    <th class="th-base">名字</th>
                    <th class="th-base">帳號</th>
                    <th class="th-base">Email</th>
                    <th class="th-base">電話</th>
                    <th class="th-base">狀態</th>
                    <th class="th-base">最後登入時間</th>
                    <th class="th-base">操作</th>
                  </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="verifier in dataList" :key="verifier?.id || Math.random()" class="hover:bg-gray-50">
                    <td class="td-base">
                      <div class="flex items-center">
                        <div class="flex-shrink-0 h-10 w-10">
                          <div class="h-10 w-10 rounded-full bg-gradient-to-r from-blue-500 to-blue-600 flex items-center justify-center">
                            <span class="text-white font-semibold text-sm">{{ (verifier.name || verifier.account || 'V').charAt(0).toUpperCase() }}</span>
                          </div>
                        </div>
                        <div class="ml-4">
                          <div class="text-sm font-medium text-gray-900">{{ verifier.name || '未知名字' }}</div>
                        </div>
                      </div>
                    </td>
                    <td class="td-base">
                      <div class="text-sm text-gray-900">{{ verifier.account || '未知帳號' }}</div>
                    </td>
                    ...
                    <td class="td-base">
                      <span v-if="verifier.status === 'active'" class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">
                        啟用
                      </span>
                    ...
                    <td class="td-base text-gray-500">
                      {{ formatDateTime(verifier.last_login_at) }}
                    </td>
                    <td class="td-base font-medium">
                      <div class="action-buttons">
                        <button
                          @click="openEditModal(verifier)"
                          class="action-btn action-btn--edit"
                          :disabled="!verifier?.id"
                          title="編輯核銷人員"
                        >
                          <i class="fas fa-edit action-icon"></i>
                        </button>
                        <button
                          @click="confirmDelete(verifier)"
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  <div>
                    <p class="text-sm text-gray-700">
                      顯示第 <span class="font-medium">{{ pagination.from || 0 }}</span> 到 <span class="font-medium">{{ pagination.to || 0 }}</span> 筆，共 <span class="font-medium">{{ pagination.total || 0 }}</span> 筆
                    </p>
                  </div>
                  <div>
        ...
        <div class="mt-3">
          ...
          <form @submit.prevent="submitForm" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label class="label-base">名字 <span class="text-red-500">*</span></label>
                <input
                  v-model="form.name"
                  type="text"
                  required
              ...
              <div>
                <label class="label-base">帳號 <span class="text-red-500">*</span></label>
                <input
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  :required="!isEditing"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                  :placeholder="isEditing ? '留空表示不修改密碼' : '請輸入密碼（至少6個字元）'"
                  minlength="6"
                >
              </div>
      ...
      <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
          ...
          <p class="text-sm text-gray-600 text-center mb-6">
            您確定要刪除核銷人員「{{ verifierToDelete?.name || verifierToDelete?.account }}」嗎？此操作無法復原。
          </p>
...
import verifierAPI from '../api/verifier.js';
...
const form = ref({ id: null, name: '', account: '', email: '', phone: '', password: '', status: 'active' });

const loadVerifierDefaults = async () => {
  try {
    const resp = await verifierAPI.getVerifier(0);
    if (resp.status && resp.data) {
      form.value = { id: null, ...resp.data, password: '' };
    }
  } catch (e) {
...
// 方法
const loadVerifiers = async (page = 1) => {
  ...
  try {
    ...
    const response = await verifierAPI.getVerifiers(filters);
    ...
    if (response.status) {
      ...
      error.value = response.message || '載入核銷人員列表失敗';
      dataList.value = [];
    }
  } catch (e) {
    console.error('載入核銷人員列表錯誤:', e);
    error.value = e.message || '載入核銷人員列表失敗';
    dataList.value = [];
  } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      if (!form.value.id) {
        error.value = '無效的核銷人員 ID';
        isSubmitting.value = false;
        return;
      }
      response = await verifierAPI.updateVerifier(form.value.id, submitData);
    } else {
      response = await verifierAPI.createVerifier(submitData);
    }
...
const confirmDelete = (verifier) => {
  if (!verifier || !verifier.id) {
    error.value = '無效的核銷人員資料';
    return;
  }
  
  verifierToDelete.value = verifier;
  showDeleteModal.value = true;
};

const deleteVerifier = async () => {
  if (!verifierToDelete.value || !verifierToDelete.value.id) {
    error.value = '無效的核銷人員資料';
    return;
  }
  ...
  try {
    const response = await verifierAPI.deleteVerifier(verifierToDelete.value.id);
    if (response.status) {
